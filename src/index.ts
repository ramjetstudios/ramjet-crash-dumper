...
import ReadUnrealDump from './unreal';
...
R.post('/', async (ctx) => {
  ...
  const data = ReadUnrealDump(decompressed);
  let stack: string = '';
  ...
  if (data.files['CrashContext.runtime-xml']) {
    ...
    propertiesForDiscord['Seconds Since Start'] = xml.FGenericCrashContext.RuntimeProperties[0]['SecondsSinceStart'][0];
  }

  if (stack) {
    await SendToDiscord(data.dumpID, stack, propertiesForDiscord, data.files['Vein.log']);
  }
